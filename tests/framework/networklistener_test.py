...
class NetworkListenerTest(unittest.TestCase):
    ...
    def test_start_listening_connection_aborted(self, mock_framework):
        ...
        listener = NetworkListener(mock_config, mock_socket)
        listener.running = True
        self.assertRaises(ConnectionAbortedError, listener.start_listening,
                          mock_socket)
    ...
    def test_nl_shutdown(self, mock_join):
        ...
        listener = NetworkListener(mock_config, None)
        listener.session_socket = mock_socket
        listener.running = True

        with patch('platform.system', return_value='Darwin'):
            listener.shutdown()
        self.assertFalse(listener.running)
        self.assertFalse(mock_socket.shutdown.called)
        self.assertFalse(mock_socket.detach.called)
        self.assertTrue(mock_socket.close)
        ...
        self.assertTrue(mock_socket.detach.called)
        self.assertTrue(mock_socket.close)
